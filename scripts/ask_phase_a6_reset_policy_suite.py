#!/usr/bin/env python3
"""Generate Phase A6 reset policy source evidence (AQ6-G3)."""
...
PHASE_ID = "A6"
SOURCE = "ask_reset_policy_suite"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a6-recovery-edge-hardening/evidence/ask_reset_policy_suite.json"
)
...
def _build_payload(
    *,
    ...
    reset_policy_violation_events = 0.0
    reset_queue_clear_failures = 0.0
    reset_stale_thread_send_attempt_events = 0.0
    reset_scenarios_checked = 0.0
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_metric = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_reset_policy_violation_events",
                "context.ask_reset_policy_violation_events",
                "value",
            ],
        )
        if extracted_metric is not None and extracted_metric >= 0:
            reset_policy_violation_events = extracted_metric

        extracted_reset_queue_clear_failures = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_reset_queue_clear_failures",
                "context.ask_reset_queue_clear_failures",
            ],
        )
        if extracted_reset_queue_clear_failures is not None and extracted_reset_queue_clear_failures >= 0:
            reset_queue_clear_failures = extracted_reset_queue_clear_failures

        extracted_reset_stale_thread_send_attempt_events = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_reset_stale_thread_send_attempt_events",
                "context.ask_reset_stale_thread_send_attempt_events",
            ],
        )
        if (
            extracted_reset_stale_thread_send_attempt_events is not None
            and extracted_reset_stale_thread_send_attempt_events >= 0
        ):
            reset_stale_thread_send_attempt_events = extracted_reset_stale_thread_send_attempt_events

        extracted_reset_scenarios_checked = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_reset_scenarios_checked",
                "context.ask_reset_scenarios_checked",
            ],
        )
        if extracted_reset_scenarios_checked is not None and extracted_reset_scenarios_checked >= 0:
            reset_scenarios_checked = extracted_reset_scenarios_checked

    metric_value = round(float(reset_policy_violation_events), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "context": {
            "scenario": "ask_phase_a6_reset_policy_suite_v1",
            "ask_reset_policy_violation_events": metric_value,
            "ask_reset_queue_clear_failures": int(reset_queue_clear_failures),
            "ask_reset_stale_thread_send_attempt_events": int(reset_stale_thread_send_attempt_events),
            "ask_reset_scenarios_checked": int(reset_scenarios_checked),
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A6 reset policy source evidence generation failed: {exc}")
        return 1
