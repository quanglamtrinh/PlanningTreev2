#!/usr/bin/env python3
"""Generate Phase 13 queue state machine evidence (P13-G1 source)."""
...
PHASE_ID = "13"
SOURCE = "queue_state_machine_suite"
DEFAULT_OUTPUT = Path(
    "docs/render/phases/phase-13-queued-follow-up-flow/evidence/queue_state_machine_suite.json"
)
...
def _build_payload(
    *,
    ...
    queued_message_loss_events = 0.0
    single_flight_violation_events = 0.0
    local_storage_rehydrate_loss_events = 0.0
    idempotency_key_missing_events = 0.0

    if resolved_candidate_path is not None:
        candidate_payload = _load_json(resolved_candidate_path)
        extracted_loss = _first_number(
            candidate_payload,
            [
                "candidate_metrics.queued_message_loss_events",
                "context.queued_message_loss_events",
                "value",
            ],
        )
        if extracted_loss is not None and extracted_loss >= 0:
            queued_message_loss_events = extracted_loss
        extracted_single_flight = _first_number(
            candidate_payload,
            [
                "candidate_metrics.single_flight_violation_events",
                "context.single_flight_violation_events",
            ],
        )
        if extracted_single_flight is not None and extracted_single_flight >= 0:
            single_flight_violation_events = extracted_single_flight
        extracted_hydrate_loss = _first_number(
            candidate_payload,
            [
                "candidate_metrics.local_storage_rehydrate_loss_events",
                "context.local_storage_rehydrate_loss_events",
            ],
        )
        if extracted_hydrate_loss is not None and extracted_hydrate_loss >= 0:
            local_storage_rehydrate_loss_events = extracted_hydrate_loss
        extracted_idempotency_missing = _first_number(
            candidate_payload,
            [
                "candidate_metrics.idempotency_key_missing_events",
                "context.idempotency_key_missing_events",
            ],
        )
        if extracted_idempotency_missing is not None and extracted_idempotency_missing >= 0:
            idempotency_key_missing_events = extracted_idempotency_missing

    metric_value = round(float(queued_message_loss_events), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "pass": passed,
        "context": {
            "scenario": "phase13_queue_state_machine_suite_v1",
            "queued_message_loss_events": metric_value,
            "single_flight_violation_events": float(round(single_flight_violation_events, 3)),
            "local_storage_rehydrate_loss_events": float(round(local_storage_rehydrate_loss_events, 3)),
            "idempotency_key_missing_events": float(round(idempotency_key_missing_events, 3)),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate queue state machine JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 13 queue state machine evidence generation failed: {exc}")
        return 1
