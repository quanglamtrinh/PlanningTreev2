#!/usr/bin/env python3
"""Generate Phase 09 parse cache trace evidence (P09-G2 source)."""
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate parse cache trace evidence or use --allow-synthetic for local dry-run only."
            )
...
def _percentage(*, numerator: float, denominator: float) -> float:
    if denominator <= 0:
        raise ValueError("parse_cache_lookups must be > 0.")
...
def _build_payload(
    *,
    ...
    parse_cache_lookups = 900.0
    parse_cache_hits = 360.0

    if resolved_candidate_path is not None:
        candidate_payload = _load_json(resolved_candidate_path)
        extracted_lookups = _first_number(
            candidate_payload,
            [
                "candidate_metrics.parse_cache_lookups",
                "candidate_metrics.lookup_count",
                "parse_cache_lookups",
            ],
        )
        if extracted_lookups is not None and extracted_lookups > 0:
            parse_cache_lookups = extracted_lookups
        extracted_hits = _first_number(
            candidate_payload,
            [
                "candidate_metrics.parse_cache_hits",
                "candidate_metrics.cache_hits",
                "parse_cache_hits",
            ],
        )
        if extracted_hits is not None and extracted_hits >= 0:
            parse_cache_hits = extracted_hits
        extracted_rate = _first_number(
            candidate_payload,
            [
                "candidate_metrics.parse_cache_hit_rate_pct",
                "value",
            ],
        )
        if extracted_rate is not None and extracted_rate >= 0:
            parse_cache_hit_rate_pct = extracted_rate
        else:
            parse_cache_hit_rate_pct = _percentage(
                numerator=parse_cache_hits,
                denominator=parse_cache_lookups,
            )
    else:
        parse_cache_hit_rate_pct = _percentage(
            numerator=parse_cache_hits,
    ...
    parse_cache_hit_rate_pct = round(float(parse_cache_hit_rate_pct), 3)
    parse_cache_misses = max(0.0, parse_cache_lookups - parse_cache_hits)
    target = float(gate["target"])
    operator = str(gate["operator"])
    passed = _evaluate(operator, parse_cache_hit_rate_pct, target)
    ...
    payload: dict[str, Any] = {
        ...
        "value": parse_cache_hit_rate_pct,
        "target": target,
        "operator": operator,
        "pass": passed,
        "context": {
            "scenario": "phase09_parse_cache_trace_v1",
            "parse_cache_lookups": int(parse_cache_lookups),
            "parse_cache_hits": int(parse_cache_hits),
            "parse_cache_misses": int(parse_cache_misses),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate parse cache trace JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 09 parse cache trace generation failed: {exc}")
        return 1
