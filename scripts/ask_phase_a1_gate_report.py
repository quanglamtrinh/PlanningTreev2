#!/usr/bin/env python3
"""Evaluate Phase A1 gate pass/fail from ask source evidence JSON files."""
...
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a1-backend-ask-idempotency-foundation/evidence/phase-a1-gate-report.json"
)
...
def _phase_gates(gates_file: Path) -> list[dict[str, Any]]:
    ...
    for gate in phase_gates:
        ...
        if not gate_id or not metric or not source:
            raise ValueError("phase-gates contains invalid gate record for phase A1.")
    ...
    required_sources = {
        "ask_idempotency_integration",
        "ask_retry_replay_suite",
        "ask_start_turn_latency_probe",
    }
    actual_sources = {str(g["source"]) for g in resolved}
    missing = sorted(required_sources - actual_sources)
    if missing:
        raise ValueError(f"phase-gates missing required phase A1 sources: {', '.join(missing)}")
    return resolved
...
def _build_report(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "metric": "phase_a1_gate_pass_count",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A1 gate evaluation failed: {exc}")
        return 1
