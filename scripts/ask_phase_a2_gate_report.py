#!/usr/bin/env python3
"""Evaluate Phase A2 gate pass/fail from ask source evidence JSON files."""
...
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a2-lane-aware-queue-core-refactor/evidence/phase-a2-gate-report.json"
)
...
def _phase_gates(gates_file: Path) -> list[dict[str, Any]]:
    ...
    for gate in phase_gates:
        ...
        if not gate_id or not metric or not source:
            raise ValueError("phase-gates contains invalid gate record for phase A2.")
    ...
    if missing:
        raise ValueError(f"phase-gates missing required phase A2 sources: {', '.join(missing)}")
    return resolved
...
def _build_report(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "metric": "phase_a2_gate_pass_count",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A2 gate evaluation failed: {exc}")
        return 1
