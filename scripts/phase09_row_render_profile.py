#!/usr/bin/env python3
"""Generate Phase 09 row render profile evidence (P09-G1 source)."""
...
PHASE_ID = "09"
SOURCE = "row_render_profile"
DEFAULT_OUTPUT = Path(
    "docs/render/phases/phase-09-row-isolation-cache/evidence/row_render_profile.json"
)
DEFAULT_BASELINE = Path(
    "docs/render/phases/phase-09-row-isolation-cache/evidence/baseline-manifest-v1.json"
)
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate row render profile evidence or use --allow-synthetic for local dry-run only."
            )
...
def _percentage(*, numerator: float, denominator: float) -> float:
    if denominator <= 0:
        raise ValueError("total_row_render_events must be > 0.")
...
def _build_payload(
    *,
    ...
    total_row_render_events = 1200.0
    changed_row_render_events = 1075.0
    unchanged_row_render_events = total_row_render_events - changed_row_render_events
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_total = _first_number(
            candidate_payload,
            [
                "candidate_metrics.total_row_render_events",
                "candidate_metrics.row_render_event_count",
                "total_row_render_events",
            ],
        )
        if extracted_total is not None and extracted_total > 0:
            total_row_render_events = extracted_total
        extracted_changed = _first_number(
            candidate_payload,
            [
                "candidate_metrics.changed_row_render_events",
                "candidate_metrics.changed_render_events",
                "context.changed_row_render_events",
            ],
        )
        if extracted_changed is not None and extracted_changed >= 0:
            changed_row_render_events = extracted_changed
            unchanged_row_render_events = max(0.0, total_row_render_events - changed_row_render_events)
        extracted_unchanged = _first_number(
            candidate_payload,
            [
                "candidate_metrics.unchanged_row_render_events",
                "candidate_metrics.unchanged_render_events",
                "context.unchanged_row_render_events",
            ],
        )
        if extracted_unchanged is not None and extracted_unchanged >= 0:
            unchanged_row_render_events = extracted_unchanged
        extracted_rate = _first_number(
            candidate_payload,
            [
                "candidate_metrics.unchanged_row_rerender_rate_pct",
                "value",
            ],
        )
        if extracted_rate is not None and extracted_rate >= 0:
            unchanged_row_rerender_rate_pct = extracted_rate
        else:
            unchanged_row_rerender_rate_pct = _percentage(
                numerator=unchanged_row_render_events,
                denominator=total_row_render_events,
            )
    else:
        unchanged_row_rerender_rate_pct = _percentage(
            numerator=unchanged_row_render_events,
    ...
    unchanged_row_rerender_rate_pct = round(float(unchanged_row_rerender_rate_pct), 3)
    target = float(gate["target"])
    operator = str(gate["operator"])
    passed = _evaluate(operator, unchanged_row_rerender_rate_pct, target)
    ...
    payload: dict[str, Any] = {
        ...
        "value": unchanged_row_rerender_rate_pct,
        "target": target,
        "operator": operator,
        "pass": passed,
        "context": {
            "scenario": "phase09_row_render_profile_v1",
            "total_row_render_events": int(total_row_render_events),
            "changed_row_render_events": int(changed_row_render_events),
            "unchanged_row_render_events": int(unchanged_row_render_events),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate row render profile JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 09 row render profile generation failed: {exc}")
        return 1
