#!/usr/bin/env python3
"""Generate Phase 10 scroll smoothness evidence (P10-G2 source)."""
...
PHASE_ID = "10"
SOURCE = "scroll_smoothness_profile"
DEFAULT_OUTPUT = Path(
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate scroll smoothness evidence or use --allow-synthetic for local dry-run only."
            )
...
def _build_payload(
    *,
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_sample_count = _first_number(
            ...
            [
                ...
                "candidate_metrics.scroll_profile_samples",
                "context.sample_count",
            ],
        )
        if extracted_sample_count is not None and extracted_sample_count > 0:
            sample_count = extracted_sample_count
        extracted_total_frames = _first_number(
            candidate_payload,
            [
                "candidate_metrics.total_frames",
                "context.total_frames",
            ],
        )
        if extracted_total_frames is not None and extracted_total_frames > 0:
            total_frames = extracted_total_frames
        extracted_jank_frames = _first_number(
            candidate_payload,
            [
                "candidate_metrics.jank_frames",
                "context.jank_frames",
            ],
        )
        if extracted_jank_frames is not None and extracted_jank_frames >= 0:
            jank_frames = extracted_jank_frames
        extracted_rate = _first_number(
            candidate_payload,
            [
                "candidate_metrics.scroll_jank_frames_pct",
                "context.scroll_jank_frames_pct",
                "value",
            ],
        )
        if extracted_rate is not None and extracted_rate >= 0:
            scroll_jank_frames_pct = extracted_rate
        else:
            scroll_jank_frames_pct = _percentage(numerator=jank_frames, denominator=total_frames)
    else:
        scroll_jank_frames_pct = _percentage(numerator=jank_frames, denominator=total_frames)

    scroll_jank_frames_pct = round(float(scroll_jank_frames_pct), 3)
    target = float(gate["target"])
    operator = str(gate["operator"])
    passed = _evaluate(operator, scroll_jank_frames_pct, target)
    ...
    payload: dict[str, Any] = {
        ...
        "value": scroll_jank_frames_pct,
        "target": target,
        "operator": operator,
        "pass": passed,
        "context": {
            "scenario": "phase10_scroll_smoothness_profile_v1",
            "sample_count": int(sample_count),
            "total_frames": int(total_frames),
            "jank_frames": int(jank_frames),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate scroll smoothness JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 10 scroll smoothness profile generation failed: {exc}")
        return 1
