#!/usr/bin/env python3
"""Generate Phase A6 reconnect duplicate-guard source evidence (AQ6-G2)."""
...
PHASE_ID = "A6"
SOURCE = "ask_reconnect_duplicate_guard_suite"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a6-recovery-edge-hardening/evidence/ask_reconnect_duplicate_guard_suite.json"
)
DEFAULT_BASELINE = Path(
    "docs/render/ask-migration-phases/phase-a6-recovery-edge-hardening/evidence/baseline-manifest-v1.json"
)
...
def _build_payload(
    *,
    ...
    reconnect_duplicate_send_events = 0.0
    reconnect_cycles_checked = 0.0
    reconnect_replay_overlap_duplicate_events = 0.0
    reconnect_idempotency_collision_events = 0.0
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_metric = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_reconnect_duplicate_send_events",
                "context.ask_reconnect_duplicate_send_events",
                "value",
            ],
        )
        if extracted_metric is not None and extracted_metric >= 0:
            reconnect_duplicate_send_events = extracted_metric

        extracted_reconnect_cycles_checked = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_reconnect_cycles_checked",
                "context.ask_reconnect_cycles_checked",
            ],
        )
        if extracted_reconnect_cycles_checked is not None and extracted_reconnect_cycles_checked >= 0:
            reconnect_cycles_checked = extracted_reconnect_cycles_checked

        extracted_replay_overlap_duplicate_events = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_reconnect_replay_overlap_duplicate_events",
                "context.ask_reconnect_replay_overlap_duplicate_events",
            ],
        )
        if extracted_replay_overlap_duplicate_events is not None and extracted_replay_overlap_duplicate_events >= 0:
            reconnect_replay_overlap_duplicate_events = extracted_replay_overlap_duplicate_events

        extracted_idempotency_collision_events = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_reconnect_idempotency_collision_events",
                "context.ask_reconnect_idempotency_collision_events",
            ],
        )
        if extracted_idempotency_collision_events is not None and extracted_idempotency_collision_events >= 0:
            reconnect_idempotency_collision_events = extracted_idempotency_collision_events

    metric_value = round(float(reconnect_duplicate_send_events), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "context": {
            "scenario": "ask_phase_a6_reconnect_duplicate_guard_suite_v1",
            "ask_reconnect_duplicate_send_events": metric_value,
            "ask_reconnect_cycles_checked": int(reconnect_cycles_checked),
            "ask_reconnect_replay_overlap_duplicate_events": int(reconnect_replay_overlap_duplicate_events),
            "ask_reconnect_idempotency_collision_events": int(reconnect_idempotency_collision_events),
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A6 reconnect duplicate-guard source evidence generation failed: {exc}")
        return 1
