#!/usr/bin/env python3
"""Generate Phase 12 preview-to-full navigation evidence (P12-G3 source)."""
...
DEFAULT_OUTPUT = Path(
    "docs/render/phases/phase-12-data-volume-and-heavy-content-ux/evidence/preview_to_full_navigation_tests.json"
)
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate preview/full evidence or use --allow-synthetic for local dry-run only."
            )
...
def _build_payload(
    *,
    ...
    full_artifact_access_failures = 0.0
    preview_click_count = 58.0
    full_view_open_count = 58.0
    modal_close_success_count = 58.0
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_failures = _first_number(
            candidate_payload,
            [
                "candidate_metrics.full_artifact_access_failures",
                "context.full_artifact_access_failures",
                "value",
            ],
        )
        if extracted_failures is not None and extracted_failures >= 0:
            full_artifact_access_failures = extracted_failures
        extracted_preview_clicks = _first_number(
            candidate_payload,
            [
                "candidate_metrics.preview_click_count",
                "context.preview_click_count",
            ],
        )
        if extracted_preview_clicks is not None and extracted_preview_clicks >= 0:
            preview_click_count = extracted_preview_clicks
        extracted_open_count = _first_number(
            candidate_payload,
            [
                "candidate_metrics.full_view_open_count",
                "context.full_view_open_count",
            ],
        )
        if extracted_open_count is not None and extracted_open_count >= 0:
            full_view_open_count = extracted_open_count
        extracted_close_count = _first_number(
            candidate_payload,
            [
                "candidate_metrics.modal_close_success_count",
                "context.modal_close_success_count",
            ],
        )
        if extracted_close_count is not None and extracted_close_count >= 0:
            modal_close_success_count = extracted_close_count

    metric_value = round(float(full_artifact_access_failures), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "pass": passed,
        "context": {
            "scenario": "phase12_preview_to_full_navigation_tests_v1",
            "full_artifact_access_failures": metric_value,
            "preview_click_count": int(preview_click_count),
            "full_view_open_count": int(full_view_open_count),
            "modal_close_success_count": int(modal_close_success_count),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate preview/full navigation JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 12 preview/full navigation evidence generation failed: {exc}")
        return 1
