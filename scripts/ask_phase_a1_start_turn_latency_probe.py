#!/usr/bin/env python3
"""Generate Phase A1 ask start-turn latency evidence (AQ1-G3 source)."""
...
PHASE_ID = "A1"
SOURCE = "ask_start_turn_latency_probe"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a1-backend-ask-idempotency-foundation/evidence/ask_start_turn_latency_probe.json"
)
    ...
    *,
    ...
    if resolved_candidate_path is not None:
        ...
        extracted = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_start_turn_latency_regression_pct",
                "context.ask_start_turn_latency_regression_pct",
                "value",
            ],
        )
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A1 ask latency evidence generation failed: {exc}")
        return 1
