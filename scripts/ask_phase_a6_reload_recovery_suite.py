#!/usr/bin/env python3
"""Generate Phase A6 reload recovery source evidence (AQ6-G1)."""
...
PHASE_ID = "A6"
SOURCE = "ask_reload_recovery_suite"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a6-recovery-edge-hardening/evidence/ask_reload_recovery_suite.json"
)
...
def _build_payload(
    *,
    ...
    reload_recovery_loss_events = 0.0
    reload_rehydrate_mismatch_events = 0.0
    reload_queue_order_violation_events = 0.0
    reload_recovery_cases_checked = 0.0
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_metric = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_reload_recovery_loss_events",
                "context.ask_reload_recovery_loss_events",
                "value",
            ],
        )
        if extracted_metric is not None and extracted_metric >= 0:
            reload_recovery_loss_events = extracted_metric

        extracted_rehydrate_mismatch_events = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_reload_rehydrate_mismatch_events",
                "context.ask_reload_rehydrate_mismatch_events",
            ],
        )
        if extracted_rehydrate_mismatch_events is not None and extracted_rehydrate_mismatch_events >= 0:
            reload_rehydrate_mismatch_events = extracted_rehydrate_mismatch_events

        extracted_queue_order_violation_events = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_reload_recovery_queue_order_violation_events",
                "context.ask_reload_recovery_queue_order_violation_events",
            ],
        )
        if extracted_queue_order_violation_events is not None and extracted_queue_order_violation_events >= 0:
            reload_queue_order_violation_events = extracted_queue_order_violation_events

        extracted_cases_checked = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_reload_recovery_cases_checked",
                "context.ask_reload_recovery_cases_checked",
            ],
        )
        if extracted_cases_checked is not None and extracted_cases_checked >= 0:
            reload_recovery_cases_checked = extracted_cases_checked

    metric_value = round(float(reload_recovery_loss_events), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "context": {
            "scenario": "ask_phase_a6_reload_recovery_suite_v1",
            "ask_reload_recovery_loss_events": metric_value,
            "ask_reload_rehydrate_mismatch_events": int(reload_rehydrate_mismatch_events),
            "ask_reload_recovery_queue_order_violation_events": int(reload_queue_order_violation_events),
            "ask_reload_recovery_cases_checked": int(reload_recovery_cases_checked),
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A6 reload recovery source evidence generation failed: {exc}")
        return 1
