#!/usr/bin/env python3
"""Generate Phase 13 queue reorder integration evidence (P13-G2 source)."""
...
PHASE_ID = "13"
SOURCE = "queue_reorder_integration"
DEFAULT_OUTPUT = Path(
    "docs/render/phases/phase-13-queued-follow-up-flow/evidence/queue_reorder_integration.json"
)
DEFAULT_BASELINE = Path(
    "docs/render/phases/phase-13-queued-follow-up-flow/evidence/baseline-manifest-v1.json"
)
...
def _build_payload(
    *,
    ...
    queue_order_violation_events = 0.0
    reorder_operations = 0.0
    remove_operations = 0.0
    send_now_operations = 0.0
    cap_profiles_covered = ["low", "standard", "high"]

    if resolved_candidate_path is not None:
        candidate_payload = _load_json(resolved_candidate_path)
        extracted_violations = _first_number(
            candidate_payload,
            [
                "candidate_metrics.queue_order_violation_events",
                "context.queue_order_violation_events",
                "value",
            ],
        )
        if extracted_violations is not None and extracted_violations >= 0:
            queue_order_violation_events = extracted_violations
        extracted_reorder_ops = _first_number(
            candidate_payload,
            [
                "candidate_metrics.reorder_operations",
                "context.reorder_operations",
            ],
        )
        if extracted_reorder_ops is not None and extracted_reorder_ops >= 0:
            reorder_operations = extracted_reorder_ops
        extracted_remove_ops = _first_number(
            candidate_payload,
            [
                "candidate_metrics.remove_operations",
                "context.remove_operations",
            ],
        )
        if extracted_remove_ops is not None and extracted_remove_ops >= 0:
            remove_operations = extracted_remove_ops
        extracted_send_now_ops = _first_number(
            candidate_payload,
            [
                "candidate_metrics.send_now_operations",
                "context.send_now_operations",
            ],
        )
        if extracted_send_now_ops is not None and extracted_send_now_ops >= 0:
            send_now_operations = extracted_send_now_ops

        profile_value = _dig(candidate_payload, "candidate_metrics.cap_profiles_covered") or _dig(
            candidate_payload, "context.cap_profiles_covered"
        )
        if isinstance(profile_value, list):
            normalized_profiles = []
            for raw in profile_value:
                candidate_profile = str(raw or "").strip().lower()
                if candidate_profile in ALLOWED_PROFILES and candidate_profile not in normalized_profiles:
                    normalized_profiles.append(candidate_profile)
            if normalized_profiles:
                cap_profiles_covered = normalized_profiles

    metric_value = round(float(queue_order_violation_events), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "pass": passed,
        "context": {
            "scenario": "phase13_queue_reorder_integration_v1",
            "queue_order_violation_events": metric_value,
            "reorder_operations": int(reorder_operations),
            "remove_operations": int(remove_operations),
            "send_now_operations": int(send_now_operations),
            "cap_profiles_covered": cap_profiles_covered,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate queue reorder integration JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 13 queue reorder integration evidence generation failed: {exc}")
        return 1
