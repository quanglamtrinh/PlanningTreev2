#!/usr/bin/env python3
"""Generate Phase A2 source evidence JSON for one AQ2 gate source."""
...
DEFAULT_BASELINE = Path(
    "docs/render/ask-migration-phases/phase-a2-lane-aware-queue-core-refactor/evidence/baseline-manifest-v1.json"
)
DEFAULT_GATES_FILE = Path("docs/render/ask-migration-phases/system-freeze/phase-gates-v1.json")
ALLOWED_OPERATORS = {"gte", "lte", "eq"}
SOURCE_CONFIG = {
    "execution_queue_regression_suite": {
        "output": Path(
            "docs/render/ask-migration-phases/phase-a2-lane-aware-queue-core-refactor/evidence/"
            "execution_queue_regression_suite.json"
        ),
        "default_candidate": Path(
            "docs/render/ask-migration-phases/phase-a2-lane-aware-queue-core-refactor/evidence/candidates/"
            "execution_queue_regression_suite-candidate.json"
        ),
        "metric_keys": [
            "candidate_metrics.execution_queue_parity_failures",
            "context.execution_queue_parity_failures",
            "value",
        ],
    },
    "lane_adapter_contract_tests": {
        "output": Path(
            "docs/render/ask-migration-phases/phase-a2-lane-aware-queue-core-refactor/evidence/"
            "lane_adapter_contract_tests.json"
        ),
        "default_candidate": Path(
            "docs/render/ask-migration-phases/phase-a2-lane-aware-queue-core-refactor/evidence/candidates/"
            "lane_adapter_contract_tests-candidate.json"
        ),
        "metric_keys": [
            "candidate_metrics.lane_adapter_contract_pass_rate_pct",
            "context.lane_adapter_contract_pass_rate_pct",
            "value",
        ],
    },
    "queue_state_machine_determinism": {
        "output": Path(
            "docs/render/ask-migration-phases/phase-a2-lane-aware-queue-core-refactor/evidence/"
            "queue_state_machine_determinism.json"
        ),
        "default_candidate": Path(
            "docs/render/ask-migration-phases/phase-a2-lane-aware-queue-core-refactor/evidence/candidates/"
            "queue_state_machine_determinism-candidate.json"
        ),
        "metric_keys": [
            "candidate_metrics.queue_state_transition_nondeterministic_cases",
            "context.queue_state_transition_nondeterministic_cases",
            "value",
        ],
    },
}
...
def _gate_contract(gates_file: Path, source: str) -> dict[str, Any]:
...
def _resolve_evidence_inputs(
    *,
    ...
    if resolved_candidate is None:
        default_candidate = SOURCE_CONFIG[source]["default_candidate"]
        if default_candidate.exists():
            resolved_candidate = default_candidate
    ...
    if not resolved_candidate.exists():
        raise ValueError(f"Candidate file not found: {resolved_candidate}")
    if not normalized_sha:
        raise ValueError(
            "Missing candidate commit SHA. Provide --candidate-commit-sha or set PTM_CANDIDATE_COMMIT_SHA."
        )
    return ("candidate", True, resolved_candidate, normalized_sha)
...
def _build_payload(
    *,
    ...
    if resolved_candidate_path is not None:
        ...
        extracted = _first_number(candidate_payload, SOURCE_CONFIG[source]["metric_keys"])
...
def _validate_contract(payload: dict[str, Any], source: str, phase_id: str) -> None:
    required_fields = [
        "phase",
        "gate_id",
        "source",
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="AQ2 source artifact id from phase-gates-v1.json.",
    )
    parser.add_argument("--output", type=Path, help="Output JSON path. Defaults to canonical path by source.")
    ...
    parser.add_argument(
        ...
        type=str,
        help="Candidate commit SHA (required with candidate evidence). Can also come from PTM_CANDIDATE_COMMIT_SHA.",
    )
    parser.add_argument(
        "--allow-synthetic",
        action="store_true",
...
def main() -> int:
    ...
    output_path = args.output or SOURCE_CONFIG[source]["output"]
    try:
        gate = _gate_contract(args.gates_file, source)
        ...
        if args.self_test:
            _validate_contract(payload, source, PHASE_ID)
    except Exception as exc:
        print(f"Phase A2 source evidence generation failed: {exc}")
        return 1
