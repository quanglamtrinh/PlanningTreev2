#!/usr/bin/env python3
"""Generate Phase 11 heavy payload profile evidence (P11-G1 source)."""
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate heavy payload profile evidence or use --allow-synthetic for local dry-run only."
            )
...
def _build_payload(
    *,
    ...
    reduction_pct = 52.0
    long_task_count_before = 44.0
    long_task_count_after = 21.0
    sample_count = 24.0

    if resolved_candidate_path is not None:
        candidate_payload = _load_json(resolved_candidate_path)
        extracted_reduction = _first_number(
            candidate_payload,
            [
                "candidate_metrics.main_thread_long_task_reduction_pct",
                "candidate_metrics.long_task_reduction_pct",
                "value",
            ],
        )
        if extracted_reduction is not None and extracted_reduction >= 0:
            reduction_pct = extracted_reduction
        extracted_before = _first_number(
            candidate_payload,
            [
                "candidate_metrics.long_task_count_before",
                "context.long_task_count_before",
            ],
        )
        if extracted_before is not None and extracted_before >= 0:
            long_task_count_before = extracted_before
        extracted_after = _first_number(
            candidate_payload,
            [
                "candidate_metrics.long_task_count_after",
                "context.long_task_count_after",
            ],
        )
        if extracted_after is not None and extracted_after >= 0:
            long_task_count_after = extracted_after
    ...
    reduction_pct = round(float(reduction_pct), 3)
    target = float(gate["target"])
    operator = str(gate["operator"])
    passed = _evaluate(operator, reduction_pct, target)
    ...
    payload: dict[str, Any] = {
        ...
        "pass": passed,
        "context": {
            "scenario": "phase11_heavy_payload_profile_v1",
            "sample_count": int(sample_count),
            "long_task_count_before": int(long_task_count_before),
            "long_task_count_after": int(long_task_count_after),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate heavy payload profile JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 11 heavy payload profile generation failed: {exc}")
        return 1
