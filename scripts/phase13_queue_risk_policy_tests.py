#!/usr/bin/env python3
"""Generate Phase 13 queue risk-policy evidence (P13-G3 source)."""
...
PHASE_ID = "13"
SOURCE = "queue_risk_policy_tests"
DEFAULT_OUTPUT = Path(
    "docs/render/phases/phase-13-queued-follow-up-flow/evidence/queue_risk_policy_tests.json"
)
...
def _build_payload(
    *,
    ...
    context_change_confirmation_cases = 0.0
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_unconfirmed = _first_number(
            candidate_payload,
            [
                "candidate_metrics.stale_intent_unconfirmed_send_events",
                "context.stale_intent_unconfirmed_send_events",
                "value",
            ],
        )
        if extracted_unconfirmed is not None and extracted_unconfirmed >= 0:
            stale_intent_unconfirmed_send_events = extracted_unconfirmed
        ...
        extracted_context_change_cases = _first_number(
            candidate_payload,
            [
                "candidate_metrics.context_change_confirmation_cases",
                "context.context_change_confirmation_cases",
            ],
        )
        if extracted_context_change_cases is not None and extracted_context_change_cases >= 0:
            context_change_confirmation_cases = extracted_context_change_cases
    ...
    payload: dict[str, Any] = {
        ...
        "pass": passed,
        "context": {
            "scenario": "phase13_queue_risk_policy_tests_v1",
            "stale_intent_unconfirmed_send_events": metric_value,
            "stale_age_confirmation_cases": int(stale_age_confirmation_cases),
            "context_change_confirmation_cases": int(context_change_confirmation_cases),
            "requires_confirmation_auto_send_events": float(round(requires_confirmation_auto_send_events, 3)),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate queue risk-policy JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 13 queue risk policy evidence generation failed: {exc}")
        return 1
