#!/usr/bin/env python3
"""Generate Phase A4 confirmation-noise source evidence (AQ4-G3)."""
...
PHASE_ID = "A4"
SOURCE = "ask_confirmation_noise_audit"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a4-ask-risk-confirmation-policy/evidence/ask_confirmation_noise_audit.json"
)
...
def _build_payload(
    *,
    ...
    total_confirmation_events = 0.0
    false_positive_events = 0.0
    reviewed_sessions = 0.0
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_metric = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_false_positive_confirmation_rate_pct",
                "context.ask_false_positive_confirmation_rate_pct",
                "value",
            ],
        )
        if extracted_metric is not None and extracted_metric >= 0:
            false_positive_rate_pct = extracted_metric

        extracted_total = _first_number(
            candidate_payload,
            [
                "candidate_metrics.total_confirmation_events",
                "context.total_confirmation_events",
            ],
        )
        if extracted_total is not None and extracted_total >= 0:
            total_confirmation_events = extracted_total

        extracted_false_positive_events = _first_number(
            candidate_payload,
            [
                "candidate_metrics.false_positive_events",
                "context.false_positive_events",
            ],
        )
        if extracted_false_positive_events is not None and extracted_false_positive_events >= 0:
            false_positive_events = extracted_false_positive_events

        extracted_reviewed_sessions = _first_number(
            candidate_payload,
            [
                "candidate_metrics.reviewed_sessions",
                "context.reviewed_sessions",
            ],
        )
        if extracted_reviewed_sessions is not None and extracted_reviewed_sessions >= 0:
            reviewed_sessions = extracted_reviewed_sessions

    metric_value = round(float(false_positive_rate_pct), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "context": {
            "scenario": "ask_phase_a4_confirmation_noise_audit_v1",
            "ask_false_positive_confirmation_rate_pct": metric_value,
            "total_confirmation_events": int(total_confirmation_events),
            "false_positive_events": int(false_positive_events),
            "reviewed_sessions": int(reviewed_sessions),
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A4 confirmation noise source evidence generation failed: {exc}")
        return 1
