#!/usr/bin/env python3
"""Evaluate Phase 09 gate pass/fail from source evidence JSON files."""
...
def _phase_gates(gates_file: Path) -> list[dict[str, Any]]:
    ...
    for gate in phase_gates:
        ...
        if not gate_id or not metric or not source:
            raise ValueError("phase-gates contains invalid gate record for phase 09.")
    ...
    if missing:
        raise ValueError(f"phase-gates missing required phase 09 sources: {', '.join(missing)}")
    return resolved
...
def _build_report(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "metric": "phase09_gate_pass_count",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 09 gate evaluation failed: {exc}")
        return 1
