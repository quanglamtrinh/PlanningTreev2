#!/usr/bin/env python3
"""Generate Phase A7 canary stability source evidence (AQ7-G2)."""
...
PHASE_ID = "A7"
SOURCE = "ask_canary_stability_report"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a7-test-matrix-controlled-enablement/evidence/ask_canary_stability_report.json"
)
...
def _build_payload(
    *,
    ...
    canary_blocker_incidents = 0.0
    canary_sessions_checked = 0.0
    canary_window_hours = 0.0
    canary_execution_regression_events = 0.0

    if resolved_candidate_path is not None:
        candidate_payload = _load_json(resolved_candidate_path)
        extracted_blockers = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_canary_blocker_incidents",
                "context.ask_canary_blocker_incidents",
                "value",
            ],
        )
        if extracted_blockers is not None and extracted_blockers >= 0:
            canary_blocker_incidents = extracted_blockers

        extracted_sessions = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_canary_sessions_checked",
                "context.ask_canary_sessions_checked",
            ],
        )
        if extracted_sessions is not None and extracted_sessions >= 0:
            canary_sessions_checked = extracted_sessions

        extracted_window_hours = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_canary_window_hours",
                "context.ask_canary_window_hours",
            ],
        )
        if extracted_window_hours is not None and extracted_window_hours >= 0:
            canary_window_hours = extracted_window_hours

        extracted_execution_regressions = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_canary_execution_regression_events",
                "context.ask_canary_execution_regression_events",
            ],
        )
        if extracted_execution_regressions is not None and extracted_execution_regressions >= 0:
            canary_execution_regression_events = extracted_execution_regressions

    metric_value = round(float(canary_blocker_incidents), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "context": {
            "scenario": "ask_phase_a7_canary_stability_v1",
            "ask_canary_blocker_incidents": metric_value,
            "ask_canary_sessions_checked": int(canary_sessions_checked),
            "ask_canary_window_hours": round(float(canary_window_hours), 3),
            "ask_canary_execution_regression_events": int(canary_execution_regression_events),
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A7 canary stability source evidence generation failed: {exc}")
        return 1
