#!/usr/bin/env python3
"""Generate Phase 08 reload reason audit evidence (P08-G3 source)."""
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate reload audit evidence or use --allow-synthetic for local dry-run only."
            )
...
def _build_payload(
    *,
    ...
    unclassified_reload_reason_events = 0.0
    forced_reload_event_count = 18.0
    invalid_forced_reason_count = 0.0

    if resolved_candidate_path is not None:
        candidate_payload = _load_json(resolved_candidate_path)
        extracted_unclassified = _first_number(
            candidate_payload,
            [
                "candidate_metrics.unclassified_reload_reason_events",
                "candidate_metrics.unclassified_forced_reload_reason_count",
                "unclassified_reload_reason_events",
                "value",
            ],
        )
        if extracted_unclassified is not None and extracted_unclassified >= 0:
            unclassified_reload_reason_events = extracted_unclassified
        extracted_total = _first_number(
            candidate_payload,
            [
                "candidate_metrics.forced_reload_event_count",
                "candidate_metrics.forced_reload_count",
                "context.forced_reload_event_count",
            ],
        )
        if extracted_total is not None and extracted_total >= 0:
            forced_reload_event_count = extracted_total
        extracted_invalid = _first_number(
            candidate_payload,
            [
                "candidate_metrics.invalid_forced_reason_count",
                "candidate_metrics.invalid_forced_reload_reason_count",
                "context.invalid_forced_reason_count",
            ],
        )
        if extracted_invalid is not None and extracted_invalid >= 0:
            invalid_forced_reason_count = extracted_invalid

    forced_reason_coverage_pct = 100.0
    if forced_reload_event_count > 0:
        forced_reason_coverage_pct = (
            (forced_reload_event_count - unclassified_reload_reason_events)
            / forced_reload_event_count
        ) * 100.0
    forced_reason_coverage_pct = round(forced_reason_coverage_pct, 3)

    target = float(gate["target"])
    operator = str(gate["operator"])
    metric_pass = _evaluate(operator, unclassified_reload_reason_events, target)
    invalid_reason_pass = invalid_forced_reason_count <= 0
    passed = metric_pass and invalid_reason_pass
    ...
    payload: dict[str, Any] = {
        ...
        "value": round(float(unclassified_reload_reason_events), 3),
        "target": target,
        "operator": operator,
        "pass": passed,
        "context": {
            "scenario": "phase08_reload_reason_contract_audit_v1",
            "forced_reload_event_count": int(forced_reload_event_count),
            "invalid_forced_reason_count": int(invalid_forced_reason_count),
            "forced_reason_coverage_pct": forced_reason_coverage_pct,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate reload reason audit JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 08 reload reason audit generation failed: {exc}")
        return 1
