#!/usr/bin/env python3
"""Generate Phase A1 ask retry/replay evidence (AQ1-G2 source)."""
...
PHASE_ID = "A1"
SOURCE = "ask_retry_replay_suite"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a1-backend-ask-idempotency-foundation/evidence/ask_retry_replay_suite.json"
)
    ...
    *,
    ...
    if resolved_candidate_path is not None:
        ...
        extracted = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_idempotent_replay_success_rate_pct",
                "context.ask_idempotent_replay_success_rate_pct",
                "value",
            ],
        )
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A1 ask retry/replay evidence generation failed: {exc}")
        return 1
