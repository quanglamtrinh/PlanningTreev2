#!/usr/bin/env python3
"""Generate Phase 10 virtualization anchor test evidence (P10-G3 source)."""
...
PHASE_ID = "10"
SOURCE = "virtualization_anchor_tests"
DEFAULT_OUTPUT = Path(
    "docs/render/phases/phase-10-progressive-virtualized-rendering/evidence/virtualization_anchor_tests.json"
)
DEFAULT_BASELINE = Path(
    "docs/render/phases/phase-10-progressive-virtualized-rendering/evidence/baseline-manifest-v1.json"
)
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate virtualization anchor test evidence or use --allow-synthetic for local dry-run only."
            )
...
def _build_payload(
    *,
    ...
    anchor_test_case_count = 24.0
    prepend_anchor_case_count = 10.0
    load_more_anchor_case_count = 14.0
    anchor_break_incidents = 0.0

    if resolved_candidate_path is not None:
        candidate_payload = _load_json(resolved_candidate_path)
        extracted_case_count = _first_number(
            candidate_payload,
            [
                "candidate_metrics.anchor_test_case_count",
                "context.anchor_test_case_count",
            ],
        )
        if extracted_case_count is not None and extracted_case_count > 0:
            anchor_test_case_count = extracted_case_count
        extracted_prepend_count = _first_number(
            candidate_payload,
            [
                "candidate_metrics.prepend_anchor_case_count",
                "context.prepend_anchor_case_count",
            ],
        )
        if extracted_prepend_count is not None and extracted_prepend_count >= 0:
            prepend_anchor_case_count = extracted_prepend_count
        extracted_load_more_count = _first_number(
            candidate_payload,
            [
                "candidate_metrics.load_more_anchor_case_count",
                "context.load_more_anchor_case_count",
            ],
        )
        if extracted_load_more_count is not None and extracted_load_more_count >= 0:
            load_more_anchor_case_count = extracted_load_more_count
        extracted_breaks = _first_number(
            candidate_payload,
            [
                "candidate_metrics.anchor_break_incidents",
                "context.anchor_break_incidents",
                "value",
            ],
        )
        if extracted_breaks is not None and extracted_breaks >= 0:
            anchor_break_incidents = extracted_breaks

    anchor_break_incidents = round(float(anchor_break_incidents), 3)
    target = float(gate["target"])
    operator = str(gate["operator"])
    metric_pass = _evaluate(operator, anchor_break_incidents, target)
    coverage_pass = anchor_test_case_count > 0 and (prepend_anchor_case_count + load_more_anchor_case_count) > 0
    ...
    payload: dict[str, Any] = {
        ...
        "value": anchor_break_incidents,
        "target": target,
        "operator": operator,
        "pass": passed,
        "context": {
            "scenario": "phase10_virtualization_anchor_tests_v1",
            "anchor_test_case_count": int(anchor_test_case_count),
            "prepend_anchor_case_count": int(prepend_anchor_case_count),
            "load_more_anchor_case_count": int(load_more_anchor_case_count),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate virtualization anchor tests JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 10 virtualization anchor tests generation failed: {exc}")
        return 1
