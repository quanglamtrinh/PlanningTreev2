#!/usr/bin/env python3
"""Evaluate Phase A7 gate pass/fail from ask source evidence JSON files."""
...
PHASE_ID = "A7"
SOURCE = "ask_phase_gate_report"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a7-test-matrix-controlled-enablement/evidence/phase-a7-gate-report.json"
)
...
REQUIRED_SOURCES = {
    "ask_queue_acceptance_matrix",
    "ask_canary_stability_report",
    "ask_rollback_drill_report",
}
...
def _phase_gates(gates_file: Path) -> list[dict[str, Any]]:
    ...
    for gate in phase_gates:
        ...
        if not gate_id or not metric or not source:
            raise ValueError("phase-gates contains invalid gate record for phase A7.")
    ...
    if missing:
        raise ValueError(f"phase-gates missing required phase A7 sources: {', '.join(missing)}")
    return resolved
...
def _build_report(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "metric": "phase_a7_gate_pass_count",
        ...
        "context": {
            ...
            "candidate_commit_sha": str(candidate_commit_sha or "").strip() or None,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        help="Optional candidate commit SHA metadata. Can also come from PTM_CANDIDATE_COMMIT_SHA.",
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A7 gate evaluation failed: {exc}")
        return 1
