#!/usr/bin/env python3
"""Evaluate Phase 11 gate pass/fail from source evidence JSON files."""
...
def _phase_gates(gates_file: Path) -> list[dict[str, Any]]:
    ...
    for gate in phase_gates:
        ...
        if not gate_id or not metric or not source:
            raise ValueError("phase-gates contains invalid gate record for phase 11.")
    ...
    required_sources = {
        "heavy_payload_profile",
        "worker_versioning_tests",
        "heavy_content_interaction_smoke",
    }
    actual_sources = {str(g["source"]) for g in resolved}
    missing = sorted(required_sources - actual_sources)
    if missing:
        raise ValueError(f"phase-gates missing required phase 11 sources: {', '.join(missing)}")
    return resolved
...
def _build_report(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "metric": "phase11_gate_pass_count",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 11 gate evaluation failed: {exc}")
        return 1
