#!/usr/bin/env python3
"""Generate Phase 11 worker versioning evidence (P11-G2 source)."""
...
PHASE_ID = "11"
SOURCE = "worker_versioning_tests"
DEFAULT_OUTPUT = Path(
    "docs/render/phases/phase-11-heavy-compute-off-main-thread/evidence/worker_versioning_tests.json"
)
DEFAULT_BASELINE = Path(
    "docs/render/phases/phase-11-heavy-compute-off-main-thread/evidence/baseline-manifest-v1.json"
)
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate worker versioning evidence or use --allow-synthetic for local dry-run only."
            )
...
def _build_payload(
    *,
    ...
    stale_worker_result_applies = 0.0
    stale_worker_results_dropped = 10.0
    worker_apply_attempts = 80.0

    if resolved_candidate_path is not None:
        candidate_payload = _load_json(resolved_candidate_path)
        extracted_applies = _first_number(
            candidate_payload,
            [
                "candidate_metrics.stale_worker_result_applies",
                "context.stale_worker_result_applies",
                "value",
            ],
        )
        if extracted_applies is not None and extracted_applies >= 0:
            stale_worker_result_applies = extracted_applies
        extracted_dropped = _first_number(
            candidate_payload,
            [
                "candidate_metrics.stale_worker_results_dropped",
                "context.stale_worker_results_dropped",
            ],
        )
        if extracted_dropped is not None and extracted_dropped >= 0:
            stale_worker_results_dropped = extracted_dropped
        extracted_attempts = _first_number(
            candidate_payload,
            [
                "candidate_metrics.worker_apply_attempts",
                "context.worker_apply_attempts",
            ],
        )
        if extracted_attempts is not None and extracted_attempts >= 0:
            worker_apply_attempts = extracted_attempts

    stale_worker_result_applies = round(float(stale_worker_result_applies), 3)
    target = float(gate["target"])
    operator = str(gate["operator"])
    passed = _evaluate(operator, stale_worker_result_applies, target)
    ...
    payload: dict[str, Any] = {
        ...
        "value": stale_worker_result_applies,
        "target": target,
        "operator": operator,
        "pass": passed,
        "context": {
            "scenario": "phase11_worker_versioning_tests_v1",
            "stale_worker_result_applies": stale_worker_result_applies,
            "stale_worker_results_dropped": int(stale_worker_results_dropped),
            "worker_apply_attempts": int(worker_apply_attempts),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate worker versioning JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 11 worker versioning evidence generation failed: {exc}")
        return 1
