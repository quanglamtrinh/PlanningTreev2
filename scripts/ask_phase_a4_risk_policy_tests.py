#!/usr/bin/env python3
"""Generate Phase A4 risk-policy source evidence (AQ4-G1)."""
...
PHASE_ID = "A4"
SOURCE = "ask_risk_policy_tests"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a4-ask-risk-confirmation-policy/evidence/ask_risk_policy_tests.json"
)
DEFAULT_BASELINE = Path(
    "docs/render/ask-migration-phases/phase-a4-ask-risk-confirmation-policy/evidence/baseline-manifest-v1.json"
)
...
def _build_payload(
    *,
    ...
    stale_intent_unconfirmed_send_events = 0.0
    stale_age_confirmation_cases = 0.0
    thread_drift_confirmation_cases = 0.0
    snapshot_drift_confirmation_cases = 0.0
    stale_marker_confirmation_cases = 0.0
    requires_confirmation_auto_send_events = 0.0
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_metric = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_stale_intent_unconfirmed_send_events",
                "context.ask_stale_intent_unconfirmed_send_events",
                "value",
            ],
        )
        if extracted_metric is not None and extracted_metric >= 0:
            stale_intent_unconfirmed_send_events = extracted_metric

        extracted_stale_age_cases = _first_number(
            candidate_payload,
            [
                "candidate_metrics.stale_age_confirmation_cases",
                "context.stale_age_confirmation_cases",
            ],
        )
        if extracted_stale_age_cases is not None and extracted_stale_age_cases >= 0:
            stale_age_confirmation_cases = extracted_stale_age_cases

        extracted_thread_drift_cases = _first_number(
            candidate_payload,
            [
                "candidate_metrics.thread_drift_confirmation_cases",
                "context.thread_drift_confirmation_cases",
            ],
        )
        if extracted_thread_drift_cases is not None and extracted_thread_drift_cases >= 0:
            thread_drift_confirmation_cases = extracted_thread_drift_cases

        extracted_snapshot_drift_cases = _first_number(
            candidate_payload,
            [
                "candidate_metrics.snapshot_drift_confirmation_cases",
                "context.snapshot_drift_confirmation_cases",
            ],
        )
        if extracted_snapshot_drift_cases is not None and extracted_snapshot_drift_cases >= 0:
            snapshot_drift_confirmation_cases = extracted_snapshot_drift_cases

        extracted_stale_marker_cases = _first_number(
            candidate_payload,
            [
                "candidate_metrics.stale_marker_confirmation_cases",
                "context.stale_marker_confirmation_cases",
            ],
        )
        if extracted_stale_marker_cases is not None and extracted_stale_marker_cases >= 0:
            stale_marker_confirmation_cases = extracted_stale_marker_cases

        extracted_auto_send_events = _first_number(
            candidate_payload,
            [
                "candidate_metrics.requires_confirmation_auto_send_events",
                "context.requires_confirmation_auto_send_events",
            ],
        )
        if extracted_auto_send_events is not None and extracted_auto_send_events >= 0:
            requires_confirmation_auto_send_events = extracted_auto_send_events

    metric_value = round(float(stale_intent_unconfirmed_send_events), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "context": {
            "scenario": "ask_phase_a4_risk_policy_tests_v1",
            "ask_stale_intent_unconfirmed_send_events": metric_value,
            "stale_age_confirmation_cases": int(stale_age_confirmation_cases),
            "thread_drift_confirmation_cases": int(thread_drift_confirmation_cases),
            "snapshot_drift_confirmation_cases": int(snapshot_drift_confirmation_cases),
            "stale_marker_confirmation_cases": int(stale_marker_confirmation_cases),
            "requires_confirmation_auto_send_events": float(
                round(requires_confirmation_auto_send_events, 3)
            ),
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A4 risk policy source evidence generation failed: {exc}")
        return 1
