#!/usr/bin/env python3
"""Evaluate Phase A4 gate pass/fail from ask source evidence JSON files."""
...
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a4-ask-risk-confirmation-policy/evidence/phase-a4-gate-report.json"
)
...
REQUIRED_SOURCES = {
    "ask_risk_policy_tests",
    "ask_confirmation_transition_suite",
    "ask_confirmation_noise_audit",
}
...
def _phase_gates(gates_file: Path) -> list[dict[str, Any]]:
    ...
    for gate in phase_gates:
        ...
        if not gate_id or not metric or not source:
            raise ValueError("phase-gates contains invalid gate record for phase A4.")
    ...
    if missing:
        raise ValueError(f"phase-gates missing required phase A4 sources: {', '.join(missing)}")
    return resolved
...
def _build_report(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "metric": "phase_a4_gate_pass_count",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A4 gate evaluation failed: {exc}")
        return 1
