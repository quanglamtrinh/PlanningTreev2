#!/usr/bin/env python3
"""Evaluate Phase A3 gate pass/fail from ask source evidence JSON files."""
...
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a3-ask-queue-mvp-auto-flush/evidence/phase-a3-gate-report.json"
)
DEFAULT_BASELINE = Path(
    "docs/render/ask-migration-phases/phase-a3-ask-queue-mvp-auto-flush/evidence/baseline-manifest-v1.json"
)
DEFAULT_GATES_FILE = Path("docs/render/ask-migration-phases/system-freeze/phase-gates-v1.json")
ALLOWED_OPERATORS = {"gte", "lte", "eq"}
REQUIRED_SOURCES = {
    "ask_queue_state_machine_suite",
    "ask_queue_order_integration",
    "ask_auto_flush_scenarios",
}
...
def _phase_gates(gates_file: Path) -> list[dict[str, Any]]:
    ...
    for gate in phase_gates:
        ...
        if not gate_id or not metric or not source:
            raise ValueError("phase-gates contains invalid gate record for phase A3.")
    ...
    if missing:
        raise ValueError(f"phase-gates missing required phase A3 sources: {', '.join(missing)}")
    return resolved
...
def _build_report(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "metric": "phase_a3_gate_pass_count",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A3 gate evaluation failed: {exc}")
        return 1
