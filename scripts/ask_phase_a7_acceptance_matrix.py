#!/usr/bin/env python3
"""Generate Phase A7 acceptance matrix source evidence (AQ7-G1)."""
...
PHASE_ID = "A7"
SOURCE = "ask_queue_acceptance_matrix"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a7-test-matrix-controlled-enablement/evidence/ask_queue_acceptance_matrix.json"
)
...
def _build_payload(
    *,
    ...
    acceptance_pass_rate_pct = 100.0
    acceptance_cases_total = 0.0
    acceptance_cases_passed = 0.0
    acceptance_cases_failed = 0.0

    if resolved_candidate_path is not None:
        candidate_payload = _load_json(resolved_candidate_path)
        extracted_pass_rate = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_queue_acceptance_suite_pass_rate_pct",
                "context.ask_queue_acceptance_suite_pass_rate_pct",
                "value",
            ],
        )
        if extracted_pass_rate is not None and extracted_pass_rate >= 0:
            acceptance_pass_rate_pct = extracted_pass_rate

        extracted_total = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_queue_acceptance_cases_total",
                "context.ask_queue_acceptance_cases_total",
            ],
        )
        if extracted_total is not None and extracted_total >= 0:
            acceptance_cases_total = extracted_total

        extracted_passed = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_queue_acceptance_cases_passed",
                "context.ask_queue_acceptance_cases_passed",
            ],
        )
        if extracted_passed is not None and extracted_passed >= 0:
            acceptance_cases_passed = extracted_passed

        extracted_failed = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_queue_acceptance_cases_failed",
                "context.ask_queue_acceptance_cases_failed",
            ],
        )
        if extracted_failed is not None and extracted_failed >= 0:
            acceptance_cases_failed = extracted_failed

    metric_value = round(float(acceptance_pass_rate_pct), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "context": {
            "scenario": "ask_phase_a7_acceptance_matrix_v1",
            "ask_queue_acceptance_suite_pass_rate_pct": metric_value,
            "ask_queue_acceptance_cases_total": int(acceptance_cases_total),
            "ask_queue_acceptance_cases_passed": int(acceptance_cases_passed),
            "ask_queue_acceptance_cases_failed": int(acceptance_cases_failed),
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A7 acceptance matrix source evidence generation failed: {exc}")
        return 1
