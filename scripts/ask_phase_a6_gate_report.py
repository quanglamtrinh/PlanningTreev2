#!/usr/bin/env python3
"""Evaluate Phase A6 gate pass/fail from ask source evidence JSON files."""
...
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a6-recovery-edge-hardening/evidence/phase-a6-gate-report.json"
)
...
REQUIRED_SOURCES = {
    "ask_reload_recovery_suite",
    "ask_reconnect_duplicate_guard_suite",
    "ask_reset_policy_suite",
}
...
def _phase_gates(gates_file: Path) -> list[dict[str, Any]]:
    ...
    for gate in phase_gates:
        ...
        if not gate_id or not metric or not source:
            raise ValueError("phase-gates contains invalid gate record for phase A6.")
    ...
    if missing:
        raise ValueError(f"phase-gates missing required phase A6 sources: {', '.join(missing)}")
    return resolved
...
def _build_report(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "metric": "phase_a6_gate_pass_count",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A6 gate evaluation failed: {exc}")
        return 1
