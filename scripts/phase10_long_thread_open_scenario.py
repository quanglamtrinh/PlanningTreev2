#!/usr/bin/env python3
"""Generate Phase 10 long-thread open scenario evidence (P10-G1 source)."""
...
PHASE_ID = "10"
SOURCE = "long_thread_open_scenario"
DEFAULT_OUTPUT = Path(
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate long-thread open evidence or use --allow-synthetic for local dry-run only."
            )
...
def _build_payload(
    *,
    ...
    sample_count = 24.0
    long_thread_open_tti_p95_ms = 1950.0
    long_thread_open_tti_p50_ms = 1125.0
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_sample_count = _first_number(
            ...
            [
                ...
                "candidate_metrics.thread_open_samples",
        ...
        extracted_p95 = _first_number(
            candidate_payload,
            [
                "candidate_metrics.long_thread_open_tti_p95_ms",
                "candidate_metrics.tti_p95_ms",
                "context.long_thread_open_tti_p95_ms",
                "value",
            ],
        )
        if extracted_p95 is not None and extracted_p95 >= 0:
            long_thread_open_tti_p95_ms = extracted_p95
        extracted_p50 = _first_number(
            candidate_payload,
            [
                "candidate_metrics.long_thread_open_tti_p50_ms",
                "candidate_metrics.tti_p50_ms",
                "context.long_thread_open_tti_p50_ms",
            ],
        )
        if extracted_p50 is not None and extracted_p50 >= 0:
            long_thread_open_tti_p50_ms = extracted_p50

    long_thread_open_tti_p95_ms = round(float(long_thread_open_tti_p95_ms), 3)
    long_thread_open_tti_p50_ms = round(float(long_thread_open_tti_p50_ms), 3)
    target = float(gate["target"])
    operator = str(gate["operator"])
    passed = _evaluate(operator, long_thread_open_tti_p95_ms, target)
    ...
    payload: dict[str, Any] = {
        ...
        "value": long_thread_open_tti_p95_ms,
        "target": target,
        "operator": operator,
        "pass": passed,
        "context": {
            "scenario": "phase10_long_thread_open_scenario_v1",
            "sample_count": int(sample_count),
            "long_thread_open_tti_p95_ms": long_thread_open_tti_p95_ms,
            "long_thread_open_tti_p50_ms": long_thread_open_tti_p50_ms,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate long-thread open scenario JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 10 long-thread open scenario generation failed: {exc}")
        return 1
