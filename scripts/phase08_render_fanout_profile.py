#!/usr/bin/env python3
"""Generate Phase 08 render fanout profile evidence (P08-G1 source)."""
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate profile evidence or use --allow-synthetic for local dry-run only."
            )
...
def _reduction_pct(*, baseline: float, candidate: float) -> float:
    if baseline <= 0:
        raise ValueError("Baseline component invalidation count must be > 0.")
...
def _build_payload(
    *,
    ...
    baseline_invalidation_count = 9500.0
    candidate_invalidation_count = 6175.0
    synthetic_workload_event_count = 1200
    synthetic_workload_component_count = 350
    ...
    if baseline_path is not None and baseline_path.exists():
        ...
        extracted = _first_number(
            baseline_payload,
            [
                "baseline_metrics.component_invalidation_count",
                "baseline_metrics.component_invalidations",
                "candidate_metrics.component_invalidation_count",
                "value",
            ],
        )
        if extracted is not None and extracted > 0:
            baseline_invalidation_count = extracted
    ...
    if resolved_candidate_path is not None:
        ...
        extracted = _first_number(
            ...
            [
                "candidate_metrics.component_invalidation_count",
                "candidate_metrics.component_invalidations",
                "component_invalidation_count",
                "value",
            ],
        )
        if extracted is not None and extracted >= 0:
            candidate_invalidation_count = extracted
        candidate_events = _first_number(
            candidate_payload,
            [
                "candidate_metrics.synthetic_workload_event_count",
                "context.synthetic_workload_event_count",
            ],
        )
        if candidate_events is not None:
            synthetic_workload_event_count = int(candidate_events)
        candidate_components = _first_number(
            candidate_payload,
            [
                "candidate_metrics.synthetic_workload_component_count",
                "context.synthetic_workload_component_count",
            ],
        )
        if candidate_components is not None:
            synthetic_workload_component_count = int(candidate_components)

    reduction = _reduction_pct(
        baseline=baseline_invalidation_count,
        candidate=candidate_invalidation_count,
    )
    ...
    payload: dict[str, Any] = {
        ...
        "pass": passed,
        "context": {
            "scenario": "phase08_selector_narrowing_chat_lane_v1",
            "synthetic_workload_event_count": int(synthetic_workload_event_count),
            "synthetic_workload_component_count": int(synthetic_workload_component_count),
            "baseline_component_invalidation_count": round(float(baseline_invalidation_count), 3),
            "candidate_component_invalidation_count": round(float(candidate_invalidation_count), 3),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate render fanout profile JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 08 render fanout profile generation failed: {exc}")
        return 1
