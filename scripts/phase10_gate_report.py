#!/usr/bin/env python3
"""Evaluate Phase 10 gate pass/fail from source evidence JSON files."""
...
def _phase_gates(gates_file: Path) -> list[dict[str, Any]]:
    ...
    for gate in phase_gates:
        ...
        if not gate_id or not metric or not source:
            raise ValueError("phase-gates contains invalid gate record for phase 10.")
    ...
    required_sources = {
        "long_thread_open_scenario",
        "scroll_smoothness_profile",
        "virtualization_anchor_tests",
    }
    actual_sources = {str(g["source"]) for g in resolved}
    missing = sorted(required_sources - actual_sources)
    if missing:
        raise ValueError(f"phase-gates missing required phase 10 sources: {', '.join(missing)}")
    return resolved
...
def _build_report(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "metric": "phase10_gate_pass_count",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 10 gate evaluation failed: {exc}")
        return 1
