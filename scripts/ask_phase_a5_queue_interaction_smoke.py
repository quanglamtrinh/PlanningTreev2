#!/usr/bin/env python3
"""Generate Phase A5 queue interaction smoke source evidence (AQ5-G3)."""
...
PHASE_ID = "A5"
SOURCE = "ask_queue_interaction_smoke"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a5-ask-queue-ui-shell-integrity/evidence/ask_queue_interaction_smoke.json"
)
...
def _build_payload(
    *,
    ...
    interaction_success_rate_pct: float | None = None
    total_interaction_checks = 0.0
    failed_interaction_checks = 0.0
    successful_interaction_checks: float | None = None
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_metric = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_queue_panel_interaction_success_rate_pct",
                "context.ask_queue_panel_interaction_success_rate_pct",
                "value",
            ],
        )
        if extracted_metric is not None and extracted_metric >= 0:
            interaction_success_rate_pct = extracted_metric

        extracted_total = _first_number(
            candidate_payload,
            [
                "candidate_metrics.total_interaction_checks",
                "context.total_interaction_checks",
            ],
        )
        if extracted_total is not None and extracted_total >= 0:
            total_interaction_checks = extracted_total

        extracted_failed = _first_number(
            candidate_payload,
            [
                "candidate_metrics.failed_interaction_checks",
                "context.failed_interaction_checks",
            ],
        )
        if extracted_failed is not None and extracted_failed >= 0:
            failed_interaction_checks = extracted_failed

        extracted_successful = _first_number(
            candidate_payload,
            [
                "candidate_metrics.successful_interaction_checks",
                "context.successful_interaction_checks",
            ],
        )
        if extracted_successful is not None and extracted_successful >= 0:
            successful_interaction_checks = extracted_successful

    target = float(gate["target"])
    if interaction_success_rate_pct is None:
        if total_interaction_checks > 0:
            passed_checks = max(total_interaction_checks - failed_interaction_checks, 0.0)
            interaction_success_rate_pct = (passed_checks / total_interaction_checks) * 100.0
        else:
            interaction_success_rate_pct = target

    if successful_interaction_checks is None:
        successful_interaction_checks = max(total_interaction_checks - failed_interaction_checks, 0.0)

    metric_value = round(float(interaction_success_rate_pct), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "context": {
            "scenario": "ask_phase_a5_queue_interaction_smoke_v1",
            "ask_queue_panel_interaction_success_rate_pct": metric_value,
            "total_interaction_checks": int(total_interaction_checks),
            "failed_interaction_checks": int(failed_interaction_checks),
            "successful_interaction_checks": int(successful_interaction_checks),
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A5 queue interaction smoke source evidence generation failed: {exc}")
        return 1
