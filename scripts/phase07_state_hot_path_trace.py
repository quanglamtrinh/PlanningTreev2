    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate trace evidence or use --allow-synthetic for local dry-run only."
            )
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate trace JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
