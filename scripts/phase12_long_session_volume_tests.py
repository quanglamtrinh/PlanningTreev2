#!/usr/bin/env python3
"""Generate Phase 12 long-session volume evidence (P12-G1 source)."""
...
DEFAULT_OUTPUT = Path(
    "docs/render/phases/phase-12-data-volume-and-heavy-content-ux/evidence/long_session_volume_tests.json"
)
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate long-session evidence or use --allow-synthetic for local dry-run only."
            )
...
def _build_payload(
    *,
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_events = _first_number(
            candidate_payload,
            [
                "candidate_metrics.live_items_exceeds_scrollback_cap_events",
                "context.live_items_exceeds_scrollback_cap_events",
                "value",
            ],
        )
        if extracted_events is not None and extracted_events >= 0:
            live_items_exceeds_events = extracted_events
        extracted_peak = _first_number(
            candidate_payload,
            [
                "candidate_metrics.live_items_peak",
                "context.live_items_peak",
            ],
        )
        ...
        extracted_trim_events = _first_number(
            candidate_payload,
            [
                "candidate_metrics.trim_events",
                "context.trim_events",
            ],
        )
        if extracted_trim_events is not None and extracted_trim_events >= 0:
            trim_events = extracted_trim_events
        extracted_total_items = _first_number(
            candidate_payload,
            [
                "candidate_metrics.long_session_total_items",
                "context.long_session_total_items",
            ],
        )
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate long-session volume JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 12 long-session volume evidence generation failed: {exc}")
        return 1
