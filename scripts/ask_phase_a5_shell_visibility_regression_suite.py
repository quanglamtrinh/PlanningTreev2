#!/usr/bin/env python3
"""Generate Phase A5 shell visibility regression source evidence (AQ5-G2)."""
...
PHASE_ID = "A5"
SOURCE = "ask_shell_visibility_regression_suite"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a5-ask-queue-ui-shell-integrity/evidence/ask_shell_visibility_regression_suite.json"
)
...
def _build_payload(
    *,
    ...
    shell_visibility_regressions = 0.0
    frame_context_visibility_regressions = 0.0
    clarify_chip_visibility_regressions = 0.0
    spec_chip_visibility_regressions = 0.0
    shell_layout_regressions = 0.0
    shell_scenarios_checked = 0.0
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_metric = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_shell_visibility_regressions",
                "context.ask_shell_visibility_regressions",
                "value",
            ],
        )
        if extracted_metric is not None and extracted_metric >= 0:
            shell_visibility_regressions = extracted_metric

        extracted_frame_context_regressions = _first_number(
            candidate_payload,
            [
                "candidate_metrics.frame_context_visibility_regressions",
                "context.frame_context_visibility_regressions",
            ],
        )
        if extracted_frame_context_regressions is not None and extracted_frame_context_regressions >= 0:
            frame_context_visibility_regressions = extracted_frame_context_regressions

        extracted_clarify_chip_regressions = _first_number(
            candidate_payload,
            [
                "candidate_metrics.clarify_chip_visibility_regressions",
                "context.clarify_chip_visibility_regressions",
            ],
        )
        if extracted_clarify_chip_regressions is not None and extracted_clarify_chip_regressions >= 0:
            clarify_chip_visibility_regressions = extracted_clarify_chip_regressions

        extracted_spec_chip_regressions = _first_number(
            candidate_payload,
            [
                "candidate_metrics.spec_chip_visibility_regressions",
                "context.spec_chip_visibility_regressions",
            ],
        )
        if extracted_spec_chip_regressions is not None and extracted_spec_chip_regressions >= 0:
            spec_chip_visibility_regressions = extracted_spec_chip_regressions

        extracted_shell_layout_regressions = _first_number(
            candidate_payload,
            [
                "candidate_metrics.shell_layout_regressions",
                "context.shell_layout_regressions",
            ],
        )
        if extracted_shell_layout_regressions is not None and extracted_shell_layout_regressions >= 0:
            shell_layout_regressions = extracted_shell_layout_regressions

        extracted_shell_scenarios_checked = _first_number(
            candidate_payload,
            [
                "candidate_metrics.shell_scenarios_checked",
                "context.shell_scenarios_checked",
            ],
        )
        if extracted_shell_scenarios_checked is not None and extracted_shell_scenarios_checked >= 0:
            shell_scenarios_checked = extracted_shell_scenarios_checked

    metric_value = round(float(shell_visibility_regressions), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "context": {
            "scenario": "ask_phase_a5_shell_visibility_regression_suite_v1",
            "ask_shell_visibility_regressions": metric_value,
            "frame_context_visibility_regressions": int(frame_context_visibility_regressions),
            "clarify_chip_visibility_regressions": int(clarify_chip_visibility_regressions),
            "spec_chip_visibility_regressions": int(spec_chip_visibility_regressions),
            "shell_layout_regressions": int(shell_layout_regressions),
            "shell_scenarios_checked": int(shell_scenarios_checked),
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A5 shell visibility regression source evidence generation failed: {exc}")
        return 1
