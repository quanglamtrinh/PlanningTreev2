#!/usr/bin/env python3
"""Generate Phase 09 UI regression suite evidence (P09-G3 source)."""
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate regression evidence or use --allow-synthetic for local dry-run only."
            )
...
def _build_payload(
    *,
    ...
    stale_render_artifact_incidents = 1.0
    regression_case_count = 42.0
    regression_case_fail_count = 1.0

    if resolved_candidate_path is not None:
        candidate_payload = _load_json(resolved_candidate_path)
        extracted_stale = _first_number(
            candidate_payload,
            [
                "candidate_metrics.stale_render_artifact_incidents",
                "candidate_metrics.stale_artifact_incidents",
                "stale_render_artifact_incidents",
                "value",
            ],
        )
        if extracted_stale is not None and extracted_stale >= 0:
            stale_render_artifact_incidents = extracted_stale
        extracted_case_count = _first_number(
            candidate_payload,
            [
                "candidate_metrics.regression_case_count",
                "context.regression_case_count",
            ],
        )
        if extracted_case_count is not None and extracted_case_count > 0:
            regression_case_count = extracted_case_count
        extracted_case_fail_count = _first_number(
            candidate_payload,
            [
                "candidate_metrics.regression_case_fail_count",
                "context.regression_case_fail_count",
            ],
        )
        if extracted_case_fail_count is not None and extracted_case_fail_count >= 0:
            regression_case_fail_count = extracted_case_fail_count

    target = float(gate["target"])
    operator = str(gate["operator"])
    metric_pass = _evaluate(operator, stale_render_artifact_incidents, target)
    regression_pass = regression_case_fail_count <= 0
    passed = metric_pass and regression_pass
    ...
    payload: dict[str, Any] = {
        ...
        "value": round(float(stale_render_artifact_incidents), 3),
        "target": target,
        "operator": operator,
        "pass": passed,
        "context": {
            "scenario": "phase09_ui_regression_suite_v1",
            "regression_case_count": int(regression_case_count),
            "regression_case_fail_count": int(regression_case_fail_count),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate UI regression suite JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 09 UI regression suite generation failed: {exc}")
        return 1
