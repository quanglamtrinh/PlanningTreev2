#!/usr/bin/env python3
"""Evaluate Phase A5 gate pass/fail from ask source evidence JSON files."""
...
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a5-ask-queue-ui-shell-integrity/evidence/phase-a5-gate-report.json"
)
...
REQUIRED_SOURCES = {
    "ask_queue_ui_actions_suite",
    "ask_shell_visibility_regression_suite",
    "ask_queue_interaction_smoke",
}
...
def _phase_gates(gates_file: Path) -> list[dict[str, Any]]:
    ...
    for gate in phase_gates:
        ...
        if not gate_id or not metric or not source:
            raise ValueError("phase-gates contains invalid gate record for phase A5.")
    ...
    if missing:
        raise ValueError(f"phase-gates missing required phase A5 sources: {', '.join(missing)}")
    return resolved
...
def _build_report(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "metric": "phase_a5_gate_pass_count",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A5 gate evaluation failed: {exc}")
        return 1
