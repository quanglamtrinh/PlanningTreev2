#!/usr/bin/env python3
"""Evaluate Phase 13 gate pass/fail from source evidence JSON files."""
...
PHASE_ID = "13"
SOURCE = "phase13_gate_report"
DEFAULT_OUTPUT = Path(
    "docs/render/phases/phase-13-queued-follow-up-flow/evidence/phase13-gate-report.json"
)
...
def _phase_gates(gates_file: Path) -> list[dict[str, Any]]:
    ...
    for gate in phase_gates:
        ...
        if not gate_id or not metric or not source:
            raise ValueError("phase-gates contains invalid gate record for phase 13.")
    ...
    required_sources = {
        "queue_state_machine_suite",
        "queue_reorder_integration",
        "queue_risk_policy_tests",
    }
    actual_sources = {str(g["source"]) for g in resolved}
    missing = sorted(required_sources - actual_sources)
    if missing:
        raise ValueError(f"phase-gates missing required phase 13 sources: {', '.join(missing)}")
    return resolved
...
def _build_report(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "metric": "phase13_gate_pass_count",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 13 gate evaluation failed: {exc}")
        return 1
