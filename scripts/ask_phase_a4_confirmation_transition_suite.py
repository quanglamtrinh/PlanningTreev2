#!/usr/bin/env python3
"""Generate Phase A4 confirmation-transition source evidence (AQ4-G2)."""
...
PHASE_ID = "A4"
SOURCE = "ask_confirmation_transition_suite"
DEFAULT_OUTPUT = Path(
    "docs/render/ask-migration-phases/phase-a4-ask-risk-confirmation-policy/evidence/ask_confirmation_transition_suite.json"
)
...
def _build_payload(
    *,
    ...
    transition_failures = 0.0
    queued_to_requires_confirmation_cases = 0.0
    confirmed_to_send_cases = 0.0
    confirm_without_send_attempt_cases = 0.0
    confirm_action_double_send_events = 0.0
    ...
    if resolved_candidate_path is not None:
        ...
        extracted_metric = _first_number(
            candidate_payload,
            [
                "candidate_metrics.ask_requires_confirmation_transition_failures",
                "context.ask_requires_confirmation_transition_failures",
                "value",
            ],
        )
        if extracted_metric is not None and extracted_metric >= 0:
            transition_failures = extracted_metric

        extracted_q_to_rc = _first_number(
            candidate_payload,
            [
                "candidate_metrics.queued_to_requires_confirmation_cases",
                "context.queued_to_requires_confirmation_cases",
            ],
        )
        if extracted_q_to_rc is not None and extracted_q_to_rc >= 0:
            queued_to_requires_confirmation_cases = extracted_q_to_rc

        extracted_confirm_send = _first_number(
            candidate_payload,
            [
                "candidate_metrics.confirmed_to_send_cases",
                "context.confirmed_to_send_cases",
            ],
        )
        if extracted_confirm_send is not None and extracted_confirm_send >= 0:
            confirmed_to_send_cases = extracted_confirm_send

        extracted_confirm_no_send = _first_number(
            candidate_payload,
            [
                "candidate_metrics.confirm_without_send_attempt_cases",
                "context.confirm_without_send_attempt_cases",
            ],
        )
        if extracted_confirm_no_send is not None and extracted_confirm_no_send >= 0:
            confirm_without_send_attempt_cases = extracted_confirm_no_send

        extracted_double_send = _first_number(
            candidate_payload,
            [
                "candidate_metrics.confirm_action_double_send_events",
                "context.confirm_action_double_send_events",
            ],
        )
        if extracted_double_send is not None and extracted_double_send >= 0:
            confirm_action_double_send_events = extracted_double_send

    metric_value = round(float(transition_failures), 3)
    ...
    payload: dict[str, Any] = {
        ...
        "context": {
            "scenario": "ask_phase_a4_confirmation_transition_suite_v1",
            "ask_requires_confirmation_transition_failures": metric_value,
            "queued_to_requires_confirmation_cases": int(queued_to_requires_confirmation_cases),
            "confirmed_to_send_cases": int(confirmed_to_send_cases),
            "confirm_without_send_attempt_cases": int(confirm_without_send_attempt_cases),
            "confirm_action_double_send_events": float(round(confirm_action_double_send_events, 3)),
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase A4 confirmation transition source evidence generation failed: {exc}")
        return 1
