#!/usr/bin/env python3
"""Generate Phase 11 heavy interaction smoke evidence (P11-G3 source)."""
...
PHASE_ID = "11"
SOURCE = "heavy_content_interaction_smoke"
DEFAULT_OUTPUT = Path(
    ...
    *,
    ...
    if candidate_path is None:
        if not allow_synthetic:
            raise ValueError(
                "Missing --candidate. Provide candidate heavy interaction smoke evidence or use --allow-synthetic for local dry-run only."
            )
...
def _build_payload(
    *,
    ...
    freeze_events_over_50ms = 0.0
    interaction_window_count = 40.0
    sample_count = 20.0

    if resolved_candidate_path is not None:
        candidate_payload = _load_json(resolved_candidate_path)
        extracted_freeze_events = _first_number(
            candidate_payload,
            [
                "candidate_metrics.interaction_freeze_events_over_50ms",
                "context.interaction_freeze_events_over_50ms",
                "value",
            ],
        )
        if extracted_freeze_events is not None and extracted_freeze_events >= 0:
            freeze_events_over_50ms = extracted_freeze_events
        extracted_windows = _first_number(
            candidate_payload,
            [
                "candidate_metrics.interaction_window_count",
                "context.interaction_window_count",
            ],
        )
        if extracted_windows is not None and extracted_windows >= 0:
            interaction_window_count = extracted_windows
        extracted_samples = _first_number(
            candidate_payload,
            [
        ...
        if extracted_samples is not None and extracted_samples > 0:
            sample_count = extracted_samples

    freeze_events_over_50ms = round(float(freeze_events_over_50ms), 3)
    target = float(gate["target"])
    operator = str(gate["operator"])
    passed = _evaluate(operator, freeze_events_over_50ms, target)
    ...
    payload: dict[str, Any] = {
        ...
        "value": freeze_events_over_50ms,
        "target": target,
        "operator": operator,
        "pass": passed,
        "context": {
            "scenario": "phase11_heavy_content_interaction_smoke_v1",
            "sample_count": int(sample_count),
            "interaction_window_count": int(interaction_window_count),
            "interaction_freeze_events_over_50ms": freeze_events_over_50ms,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--candidate",
        type=Path,
        help="Candidate heavy interaction smoke JSON path. Required unless --allow-synthetic is set.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 11 heavy interaction smoke generation failed: {exc}")
        return 1
