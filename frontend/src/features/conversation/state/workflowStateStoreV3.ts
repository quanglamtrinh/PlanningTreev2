...
type WorkflowStateStoreV3State = {
...
async function reloadWorkflowState(
  ...
  set: (
    partial:
      | Partial<WorkflowStateStoreV3State>
      | ((state: WorkflowStateStoreV3State) => Partial<WorkflowStateStoreV3State>),
  ) => void,
): Promise<NodeWorkflowView> {
  const workflowState = await api.getWorkflowStateV3(projectId, nodeId)
...
export const useWorkflowStateStoreV3 = create<WorkflowStateStoreV3State>((set) => ({
  entries: {},
  loading: {},
  errors: {},
  activeMutations: {},
  ...
  async loadWorkflowState(projectId: string, nodeId: string) {
    ...
    if (workflowStateInFlight.has(key)) {
      return await (workflowStateInFlight.get(key) as Promise<NodeWorkflowView>)
    }
    set((state) => ({
      loading: {
        ...state.loading,
    ...
    return runWorkflowMutation(
      {
        ...
        nodeId,
        action: 'finish_task',
        mutate: () => api.finishTaskWorkflowV3(projectId, nodeId, newIdempotencyKey('finish_task')),
      },
      set,
    )
  },
