...
import { appendAuthToken, buildProjectEventsUrlV3 } from '../../../api/client'
import { useDetailStateStore } from '../../../stores/detail-state-store'
import { useWorkflowStateStoreV3 } from './workflowStateStoreV3'
import { parseWorkflowEventEnvelope } from './threadEventRouter'

const RECONNECT_DELAY_MS = 1000

export function useWorkflowEventBridgeV3(
  ...
  useEffect(() => {
    ...
    const refreshWorkflowState = () => {
      void (async () => {
        try {
          await useWorkflowStateStoreV3.getState().loadWorkflowState(projectId, nodeId)
        } catch {
    ...
    const connect = () => {
      closeEventSource()
      const url = appendAuthToken(buildProjectEventsUrlV3(projectId))
