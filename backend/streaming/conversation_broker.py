...
class ConversationEventBroker:
    ...
    def publish(self, project_id: str, conversation_id: str, event: dict[str, Any]) -> None:
        with self._lock:
            subscribers = tuple(self._queues.get((project_id, conversation_id), set()))
        if not subscribers:
            return
    ...
    def subscribe(self, project_id: str, conversation_id: str) -> asyncio.Queue[dict[str, Any]]:
        subscriber = _Subscriber(queue=asyncio.Queue(), loop=asyncio.get_running_loop())
        with self._lock:
            self._queues[(project_id, conversation_id)].add(subscriber)
