...
def test_new_thread_starts_injects_context_and_persists_binding(storage, workspace_root) -> None:
    ...
    assert injected_item["id"] == "workflow-context-execution"
    assert injected_item["turnId"] == "workflow-context-execution-turn"
...
def test_changed_context_auto_updates_binding(storage, workspace_root) -> None:
    ...
    assert item["id"] == "workflow-context-execution"
    assert item["turnId"] == "workflow-context-execution-turn"
    ...
    assert '"kind":"execution_context"' in item["text"]
    assert response["binding"]["threadId"] == "thread-1"
