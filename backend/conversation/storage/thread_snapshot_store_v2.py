...
from backend.conversation.domain.types import ThreadRole, ThreadSnapshotV2, default_thread_snapshot, normalize_thread_snapshot
...
class ThreadSnapshotStoreV2:
    ...
    def _conversation_dir(self, project_id: str, node_id: str) -> Path:
        return self._project_dir(project_id) / "conversation_v2" / node_id
    ...
    def read_snapshot(self, project_id: str, node_id: str, thread_role: ThreadRole) -> ThreadSnapshotV2:
        with self._lock_registry.for_project(project_id):
            payload = load_json(self.path(project_id, node_id, thread_role), default=None)
            return normalize_thread_snapshot(
                payload,
    ...
    def write_snapshot(
        ...
        snapshot: ThreadSnapshotV2 | dict,
    ) -> ThreadSnapshotV2:
        with self._lock_registry.for_project(project_id):
            project_dir = self._project_dir(project_id)
            if not project_dir.exists():
                raise ProjectNotFound(project_id)
            normalized = normalize_thread_snapshot(
                snapshot,
                project_id=project_id,
                node_id=node_id,
                thread_role=thread_role,
    ...
    def clear_snapshot(self, project_id: str, node_id: str, thread_role: ThreadRole) -> ThreadSnapshotV2:
        snapshot = default_thread_snapshot(project_id, node_id, thread_role)
